...
KERNEL_FQ void m01338_init (KERN_ATTR_TMPS (brainwallet_tmp_t))
{
...
//     printf("key y coordinate:");
  ...
  print_u32_array_as_hex(pub_key, 17); }
    ...
    sha256_init   (&ctx);
    sha256_update (&ctx, pub_key, 33); // length of public key: 65
    sha256_final  (&ctx);
...
KERNEL_FQ void m01338_loop (KERN_ATTR_TMPS (brainwallet_tmp_t))
{
  return;
}

KERNEL_FQ void m01338_comp (KERN_ATTR_TMPS (brainwallet_tmp_t))
{
  /**
   * modifier
   */
